...
df = df.groupby(['Model', 'Patch_Size', 'Neighbour', 'OOD_RFI'], as_index=False).agg(
    {'AUROC_TRUE': ['mean', 'std'],
     'AUPRC_TRUE': ['mean', 'std'],
     'F1_TRUE': ['mean', 'std'],
...
for model in list(pd.unique(df.Model)):
    if model == 'DAE_disc':
        ...
        nln_rects0 = axs[0].bar(ind - width, df[_filter][_auroc]['mean'], width,
                                yerr=df[_filter][_auroc]['std'], label=_label)
        nln_rects1 = axs[1].bar(ind - width, df[_filter][_auprc]['mean'], width,
                                yerr=df[_filter][_auprc]['std'], label=_label)
        nln_rects2 = axs[2].bar(ind - width, df[_filter][_f1]['mean'], width,
                                yerr=df[_filter][_f1]['std'], label=_label)

    elif model == 'UNET':
        _filter = (df.Model == model) & (df.Patch_Size == 32)
        _label = 'UNET'
        _auroc, _auprc, _f1 = 'AUROC_TRUE', 'AUPRC_TRUE', 'F1_TRUE'
        unet_rects0 = axs[0].bar(ind, df[_filter][_auroc]['mean'], width,
                                 yerr=df[_filter][_auroc]['std'], label=_label)
        unet_rects1 = axs[1].bar(ind, df[_filter][_auprc]['mean'], width,
                                 yerr=df[_filter][_auprc]['std'], label=_label)
        unet_rects2 = axs[2].bar(ind, df[_filter][_f1]['mean'], width, yerr=df[_filter][_f1]['std'],
                                 label=_label)
        ...
        _label = 'AOFlagger'
        _auroc, _auprc, _f1 = 'AUROC_TRUE', 'AUPRC_TRUE', 'F1_TRUE'
        aoflagger_rects0 = axs[0].bar(ind + width, df[_filter][_auroc]['mean'], width,
                                      yerr=df[_filter][_auroc]['std'], label=_label)
        aoflagger_rects1 = axs[1].bar(ind + width, df[_filter][_auprc]['mean'], width,
                                      yerr=df[_filter][_auprc]['std'], label=_label)
        aoflagger_rects2 = axs[2].bar(ind + width, df[_filter][_f1]['mean'], width,
                                      yerr=df[_filter][_f1]['std'], label=_label)
...
axs[2].set_xticklabels(
    ['Blips', 'Narrow-band Burst', 'Broad-band Transient', 'Broad-band Continuous'], rotation=12,
    ha='right', fontsize=12)
axs[2].grid()

axs[2].legend(handles=[nln_rects1, unet_rects1, aoflagger_rects1],
              labels=['NLN', 'UNET', 'AOFlagger'], ncol=3, loc='lower center')
...
df = df.groupby(['Model', 'Patch_Size', 'Neighbour', 'RFI_Threshold'], as_index=False).agg(
    {'AUROC_TRUE': ['mean', 'std'],
     'AUPRC_TRUE': ['mean', 'std'],
     'F1_TRUE': ['mean', 'std'],
     'NLN_AUROC_TRUE': ['mean', 'std'],
...
axs[1].legend(handles=[nln_rects1, unet_rects1, aoflagger_rects1],
              labels=['NLN', 'UNET', 'AOFlagger'], ncol=3)
# loc='upper center',bbox_to_anchor=(0.5, -0.2),fancybox=False, shadow=False,
plt.tight_layout()
