...
def get_nln_metrics(model,
    ...
    for n_bour in args.neighbors:
        if args.algorithm == 'knn':
            ...
            recon_error = process(np.nanmean(recon_error, axis=tuple(range(1, recon_error.ndim))),
                                  per_image=False)
            ...
            if args.anomaly_type == 'MISO':
                add = metrics.roc_auc_score(test_labels_ == args.anomaly_class,
                                            error + dists + recon_error)
                mul = metrics.roc_auc_score(test_labels_ == args.anomaly_class,
                                            0.3 * error + 0.7 * dists)
    ...
    with open('outputs/{}/{}/{}/latent_scores.pkl'.format(model_type, args.anomaly_class,
                                                          args.model_name), 'wb') as f:
        pickle.dump(d, f)
...
def get_max_score(args):
    ...
    index_counter += 1
    print("{}/{} auc-roc = {}, dist-auc = {}, sum-auc ={}, mul-acu ={} time elapsed = {}s".format(
        index_counter,
